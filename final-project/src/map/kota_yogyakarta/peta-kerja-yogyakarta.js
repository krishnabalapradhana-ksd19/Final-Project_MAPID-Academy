...
const KAB_NAME = 'Kota Yogyakarta';
const KAB_LABEL = 'Kota Yogyakarta';

// Urutan tetap kecamatan di Kota Yogyakarta + warna kategorikal
// (dibuat otomatis dengan golden-angle hue supaya tiap kecamatan punya warna berbeda)
const KEC_ORDER = [
  'Kotagede', 'Mergangsan', 'Tegalrejo', 'Umbulharjo'
];
  ...
  app.innerHTML = `
    <header class="topbar">
      <div class="topbar-left">
        <img class="brand-logo" src="${kabLogo}" alt="Logo Kota Yogyakarta" />
        <div>
          <div class="brand-title">LBS Kota Yogyakarta</div>
    ...
    <div class="body-row">
      ...
      <aside class="panel" id="panel">
        <div class="panel-header">
          <h1 id="panel-title">Kota Yogyakarta</h1>
        ...
        <div class="panel-section">
          ...
          <div class="chip-row" id="kec-filter">
            <button class="chip active" type="button" data-kec="">Semua Kota Yogyakarta</button>
          </div>
        </div>
        ...
        <div class="stat-grid">
          ...
          <div class="stat-card">
            <div class="stat-label">% dari Kota Yogyakarta</div>
...
function setBasemap(key) {
  ...
  const beforeId = map.getLayer('lbs-yogyakarta-fill') ? 'lbs-yogyakarta-fill' : undefined;
...
function setActiveKec(kec) {
  ...
  if (map.getLayer('lbs-yogyakarta-fill')) {
    map.setFilter('lbs-yogyakarta-fill', kec ? ['==', ['get', 'WADMKC'], kec] : null);
    map.setFilter('lbs-yogyakarta-outline', kec ? ['==', ['get', 'WADMKC'], kec] : null);
  }
...
map.on('load', async () => {
  ...
  map.addLayer({
    id: 'lbs-yogyakarta-fill',
    type: 'fill',
    source: 'lbs-yogyakarta',
    paint: {
...
function setupFeaturePopup() {
  ...
  map.on('click', 'lbs-yogyakarta-fill', (e) => {
  ...
  map.on('mouseenter', 'lbs-yogyakarta-fill', () => {
    map.getCanvas().style.cursor = 'pointer';
  });
  map.on('mouseleave', 'lbs-yogyakarta-fill', () => {
