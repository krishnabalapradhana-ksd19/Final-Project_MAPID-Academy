...
const KAB_NAME = 'Gunungkidul';
const KAB_LABEL = 'Kabupaten Gunungkidul';

// Urutan tetap kecamatan di Kabupaten Gunungkidul + warna kategorikal
  ...
  app.innerHTML = `
    <header class="topbar">
      <div class="topbar-left">
        <img class="brand-logo" src="${kabLogo}" alt="Logo Kabupaten Gunungkidul" />
        <div>
          <div class="brand-title">LBS Gunungkidul</div>
    ...
    <div class="body-row">
      ...
      <aside class="panel" id="panel">
        <div class="panel-header">
          <h1 id="panel-title">Kabupaten Gunungkidul</h1>
        ...
        <div class="panel-section">
          ...
          <div class="chip-row" id="kec-filter">
            <button class="chip active" type="button" data-kec="">Semua Gunungkidul</button>
          </div>
        </div>
        ...
        <div class="stat-grid">
          ...
          <div class="stat-card">
            <div class="stat-label">% dari Gunungkidul</div>
...
function setBasemap(key) {
  ...
  const beforeId = map.getLayer('lbs-gunungkidul-fill') ? 'lbs-gunungkidul-fill' : undefined;
...
function setActiveKec(kec) {
  ...
  if (map.getLayer('lbs-gunungkidul-fill')) {
    map.setFilter('lbs-gunungkidul-fill', kec ? ['==', ['get', 'WADMKC'], kec] : null);
    map.setFilter('lbs-gunungkidul-outline', kec ? ['==', ['get', 'WADMKC'], kec] : null);
  }
...
map.on('load', async () => {
  ...
  map.addLayer({
    id: 'lbs-gunungkidul-fill',
    type: 'fill',
    source: 'lbs-gunungkidul',
    paint: {
...
function setupFeaturePopup() {
  ...
  map.on('click', 'lbs-gunungkidul-fill', (e) => {
  ...
  map.on('mouseenter', 'lbs-gunungkidul-fill', () => {
    map.getCanvas().style.cursor = 'pointer';
  });
  map.on('mouseleave', 'lbs-gunungkidul-fill', () => {
